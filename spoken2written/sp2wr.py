...
class SpokenToWritten:
    ...
    def check_front_last(self,word):
        front=""
        last=""
        if(len(word)>1):
            if word[-1]==',' or word[-1]=='.':
    ...
    def Convert(self):
        ...
        while i<no_of_words: 
            ...
            front,word,last=self.check_front_last(words_of_para[i])
            #Word of paragraph may of form ',dollars.' 
            if i+1!= no_of_words:
            #when word is of the form e.g.: two 
                front_n,next_word,last_n=self.check_front_last(words_of_para[i+1])
                if word.lower() in numbers.keys() and (next_word.lower()=='dollars' or next_word.lower()=='dollar'):
                    ...
                    #if word is a number written in characters
                    self.ouptut_para=self.ouptut_para+" "+str(numbers[word.lower()])
                    i=i+1
                else:
